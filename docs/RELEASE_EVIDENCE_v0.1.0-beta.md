...
- pytest **1552 passed / 3 skipped / 0 failed** — Python 3.12 (67.4 s) on the remediation tree
  (the pre-remediation baseline was 1481/3/0 in 59.7 s; the delta is the new pins). The 3.11 parity
  venv figure comes from CI, which runs both interpreters.
- Node harnesses: every `*.selftest.js` under `desktop/ui/` **and** `dashboard/static/` (the legacy
  widgets' 12 checks arrived with the D-07/D-08 fixes).
